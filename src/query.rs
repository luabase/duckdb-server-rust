...
pub async fn handle_cancellable_query(state: &AppState, params: QueryParams) -> Result<QueryResponse, AppError> {
    let command = &params.query_type;
    ...
    tracing::info!(
        "Cancellable Command: '{:?}', Query ID: '{}', Params: '{:?}'",
        command,
        query_id,
        params
    );
        ...
        Some(Command::Arrow) => {
            if let Some(sql) = params.sql {
                ...
                db_state
                    ...
                    .await
                    .map(QueryResponse::Arrow)
            }
            else {
                Err(anyhow::anyhow!("SQL query is required for Arrow command"))
            }
        }
        Some(Command::Json) => {
            if let Some(sql) = params.sql {
                ...
                db_state
                    ...
                    .await
                    .and_then(|json_bytes| {
                        let string = if json_bytes.is_empty() {
                            "[]".to_string()
                        }
                        else {
                ...
                Err(anyhow::anyhow!("SQL query is required for Json command"))
            }
        }
        Some(Command::Exec) => {
            if let Some(sql) = params.sql.as_deref() {
                ...
                Err(anyhow::anyhow!("SQL query is required for Exec command"))
            }
        }
        None => Err(anyhow::anyhow!("Query type is required")),
    };
    ...
    result.map_err(|e| AppError::Error(e))
}
